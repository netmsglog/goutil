...
// Done : make it thread-safe
func (m *Matcher) MatchAll(ins string) []int {
	/*	m.counter += 1
		fmt.Println("m.counter=", m.counter)  */
	var hits []int
	...
	for _, b := range in {
		...
		if n.child[c] != nil {
			...
			if f.output && !isin(hits, f.index) { // && f.counter != m.counter {
			...
			for !f.suffix.root {
				f = f.suffix
				if !isin(hits, f.index) { // f.counter != m.counter {
					hits = append(hits, f.index)
					// f.counter = m.counter
				} else {
...
// quick version, should work in parallel mode
func (m *Matcher) Match(ins string) bool {
	// m.counter += 1
	// var hits []int
	in := []byte(ins)
	n := m.root
